...
pub fn phi_cached(
    ...
    a: usize,
    primes: &[u64],
    cache: &crate::phi_cache::PhiCache,
    phi_table: &[u32],
    cbrt_x: u128,
    large_pi: &std::collections::HashMap<u128, u64>,
    memo: &mut PhiMemoMap,
    ...
    // ── Cache Walisch : O(1) pour n ≤ max_x et a ≤ max_a (= 100) ────────────
    if x <= cache.max_x as u128 && a <= cache.max_a {
        return cache.phi(x as u64, a) as u128;
    }

    // ── phi_table : O(1) pour n ≤ cbrt_x, a quelconque ──────────────────────
    if x <= cbrt_x {
        let stride = cbrt_x as usize + 1;
        return phi_table[a * stride + x as usize] as u128;
    }

    // ── Formule close pour a ≤ 5 ─────────────────────────────────────────────
    if let Some(val) = phi_small_a(x, a, primes) {
        return val;
    }
    ...
    // ── Feuille B : p_{a+1}² > x → 1 + π(x) − a ────────────────────────────
    // x > cbrt_x ici (sinon phi_table aurait répondu).
    // Deux sources pour π(x) :
    //   • x ≤ cache.max_x → cache.pi(x) (tableau sieve intégré)
    //   • x > cache.max_x → large_pi (pré-calculé par le sweep BIT)
    let next_leaf = primes
        .get(a)
        .map(|&p| (p as u128).saturating_mul(p as u128) > x)
        .unwrap_or_else(|| pa.saturating_mul(pa) > x);
    if next_leaf {
        let pi_x: u128 = if x <= cache.max_x as u128 {
            cache.pi(x as u64) as u128
        } else {
            *large_pi
                .get(&x)
                .unwrap_or_else(|| panic!("phi_cached: π({x}) non précalculé")) as u128
        };
        return 1 + pi_x - a as u128;
    }

    // ── Mémo pour les nœuds intérieurs ───────────────────────────────────────
    if let Some(&cached) = memo.get(&(x, a)) {
        return cached;
    }
    let result = phi_cached(x, a - 1, primes, cache, phi_table, cbrt_x, large_pi, memo)
        - phi_cached(x / pa, a - 1, primes, cache, phi_table, cbrt_x, large_pi, memo);
    memo.insert((x, a), result);
    result
}

/// Collecte les valeurs `n > cache_cutoff` nécessaires aux feuilles B de
/// la récursion Mapes, pour que le sweep BIT puisse précalculer π(n).
///
/// Même logique que `collect_phi_leaf_queries` mais avec `cache_cutoff`
/// (= `cache.max_x`) au lieu de `cbrt_x` comme borne basse.  Cela réduit
/// drastiquement le nombre de requêtes collectées.
pub fn collect_phi_leaf_queries_cached(
    x: u128,
    a: usize,
    primes: &[u64],
    cache_cutoff: u128,
    ...
    let mut visited: PhiMemoMap = PhiMemoMap::default();
    // Réutiliser PhiMemoMap comme visited set (value ignorée).
    collect_phi_leaf_queries_cached_rec(x, a, primes, cache_cutoff, queries, &mut visited);
}

fn collect_phi_leaf_queries_cached_rec(
    n: u128,
    a: usize,
    primes: &[u64],
    cache_cutoff: u128,
    queries: &mut std::collections::HashSet<u128>,
    visited: &mut PhiMemoMap,
) {
    if n == 0 {
        return;
    }
    // Le cache répond instantanément pour n ≤ cache_cutoff et a ≤ max_a.
    // On peut couper ici même pour a > max_a tant que n ≤ cache_cutoff,
    // car ces nœuds recurront et les branches droites n/pa ≤ cache_cutoff.
    // On coupe à n ≤ cache_cutoff (conservatif mais correct).
    if n <= cache_cutoff {
        return;
    }
    // phi_small_a gère a ≤ 5 sans appel π.
    if a <= 5 || a > primes.len() {
        return;
    }
    ...
    let pa = primes[a - 1] as u128;
    if n < pa {
        return; // feuille A : retourne 1
    }

    // Feuille B : p_{a+1}² > n → nécessite π(n).
    let next_leaf = primes
        .get(a)
        .map(|&p| (p as u128).saturating_mul(p as u128) > n)
        .unwrap_or_else(|| pa.saturating_mul(pa) > n);
    ...
    // visited réutilise PhiMemoMap ; on stocke 0 comme sentinel.
    if visited.insert((n, a), 0).is_some() {
        return;
    }

    collect_phi_leaf_queries_cached_rec(n, a - 1, primes, cache_cutoff, queries, visited);
    collect_phi_leaf_queries_cached_rec(n / pa, a - 1, primes, cache_cutoff, queries, visited);
}

// ─────────────────────────────────────────────────────────────────────────────
// phi_loop — récursion Legendre télescopée sans mémo (meissel_v3)
// ─────────────────────────────────────────────────────────────────────────────

/// Calcule φ(x, a) sans mémo HashMap, en utilisant PiTable (≤ √x) pour les
/// feuilles B et PhiCache (n ≤ x^(1/2.3), a ≤ 100) pour les petits nœuds.
///
/// **Formule télescopée** (équivalente à la récursion binaire de Legendre) :
/// `φ(x, a) = φ(x, c) − Σ_{k=c+1}^{a} φ(x/p_k, k−1)`
/// avec c = 5 (géré par `phi_small_a` en O(1)).
///
/// **Optimisation is_pix** : si x/p_k ≤ sqrt_x ET p_k² > x/p_k (c'est-à-dire
/// p_k³ > x), alors φ(x/p_k, k−1) = 1 + π(x/p_k) − (k−1) en O(1) via
/// PiTable.  Une fois déclenchée, tous les k′ > k vérifient aussi is_pix
/// (monotonie) → bascule en « boucle 2 » purement O(1).
///
/// **Aucun HashSet collect_phi, aucun HashMap large_pi.**
/// PiTable remplace les deux à lui seul.
pub fn phi_loop(
    x: u128,
    a: usize,
    primes: &[u64],
    cache: &crate::phi_cache::PhiCache,
    pi_table: &crate::pi_table::PiTable,
) -> u128 {
    phi_loop_rec(x, a, primes, cache, pi_table)
}
...
fn phi_loop_rec(
    ...
    // ── 1. PhiCache : O(1) pour n ≤ max_x et a ≤ max_a ─────────────────────
    if x <= cache.max_x as u128 && a <= cache.max_a {
        return cache.phi(x as u64, a) as u128;
    }

    // ── 2. Formule close pour a ≤ 5 ─────────────────────────────────────────
    if let Some(val) = phi_small_a(x, a, primes) {
        return val;
    }
    ...
    // ── 4. Feuille B : p_{a+1}² > x → 1 + π(x) − a ─────────────────────────
    // Uniquement si π(x) est disponible (x ≤ pi_table.max_x ou x ≤ cache.max_x).
    let next_leaf = primes
        .get(a)
        .map(|&p| (p as u128) * (p as u128) > x)
        .unwrap_or(pa * pa > x);
    if next_leaf {
        if x <= pi_table.max_x() as u128 {
            return 1 + pi_table.pi(x as u64) as u128 - a as u128;
        }
        if x <= cache.max_x as u128 {
            // a > cache.max_a ici ; cache.pi() est disponible jusqu'à cache.max_x.
            return 1 + cache.pi(x as u64) as u128 - a as u128;
        }
        // x > sqrt_x ET x > cache.max_x : tomber en récursion (correct, termine).
    }

    // ── 5. Formule télescopée : φ(x, a) = φ(x, c) − Σ_{k=c+1}^{a} φ(x/pk, k−1)
    // c = 5 (phi_small_a l'évalue en O(1)).
    const C: usize = 5;
    // φ(x, C) via phi_small_a (a > C ici).
    let base = phi_small_a(x, C, primes).unwrap();
    // Cas dégénéré (ne devrait pas arriver avec a > 5 garanti ci-dessus) :
    if a <= C {
        return base;
    }

    let mut sum: i128 = base as i128;
    let sqrt_x = pi_table.max_x(); // = ⌊√x_original⌋

    let mut k = C + 1; // p_k = primes[k-1] (indexation 1-based)
    while k <= a {
        let pk = primes[k - 1];
        let xp = x / pk as u128;

        // ── is_pix(xp, k−1) : xp ≤ √x ET pk² > xp (i.e., pk³ > x) ──────────
        // Dès que is_pix se déclenche pour k, il se déclenche pour tout k′ > k
        // (xp décroît et pk³ croît), donc on bascule en boucle 2.
        let pk_sq = (pk as u128) * (pk as u128);
        if xp <= sqrt_x as u128 && pk_sq > xp {
            // Boucle 2 : tous les k restants sont O(1) via PiTable.
            sum -= 1 + pi_table.pi(xp as u64) as i128 - (k - 1) as i128;
            k += 1;
            while k <= a {
                let pk2 = primes[k - 1];
                let xp2 = x / pk2 as u128;
                sum -= 1 + pi_table.pi(xp2 as u64) as i128 - (k - 1) as i128;
                k += 1;
            }
            break;
        }

        // ── Récursion pour φ(xp, k−1) ────────────────────────────────────────
        // Vérifications rapides avant de récurser.
        let sub = if xp <= cache.max_x as u128 && k - 1 <= cache.max_a {
            cache.phi(xp as u64, k - 1) as i128
        } else if k - 1 <= 5 {
            phi_small_a(xp, k - 1, primes).unwrap() as i128
        } else {
            phi_loop_rec(xp, k - 1, primes, cache, pi_table) as i128
        };
        sum -= sub;
        k += 1;
    }

    debug_assert!(sum >= 0, "phi_loop: résultat négatif pour x={x}, a={a}");
    sum as u128
}

// ── S1 ordinary leaves ────────────────────────────────────────────────────────

/// Public accessor for the closed-form φ(n, a) for a ≤ 5 (inclusion-exclusion).
///
/// Returns `None` if `a > 5` or `a > primes.len()`.
#[inline]
